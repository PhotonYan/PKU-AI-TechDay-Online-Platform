...
```
> 必填环境变量：`JWT_SECRET`、`ADMIN_EMAIL`、`ADMIN_PASSWORD`、`DATABASE_URL`、`ADMIN_DB_HEADER_SECRET`、`POSTS_DIR`、`UPLOADS_DIR`。本地可直接复制 `.env.example` 并按需调整，未配置将无法启动。  
> 默认使用 SQLite `backend/techday.db`。启动脚本会自动建表并（仅首次）创建管理员账号。  
...
> ⚠️ 生产部署同样需要提供 `.env`（或 Kubernetes/Docker Secrets），并挂载 `./data/uploads` 与 `./data/posts` 到容器，确保附件与 Markdown 长期保存。
