...
后端通过 Pydantic `Settings` 读取 `.env`（或环境变量），以下字段必须提供：

| 变量 | 用途 |
| --- | --- |
| `JWT_SECRET` | JWT 签名密钥 |
| `ADMIN_EMAIL` / `ADMIN_PASSWORD` | 首次启动时创建的默认管理员 |
| `DATABASE_URL` | SQLAlchemy DSN（SQLite / PostgreSQL） |
| `ADMIN_DB_HEADER_SECRET` | 数据库控制台 Header `X-Database-Password` 的密钥 |
| `POSTS_DIR` | Markdown 公告目录（默认 `/data/posts`） |
| `UPLOADS_DIR` | 上传文件目录（默认 `/data/uploads`） |
| `ENV`/`PROJECT_NAME`/`ACCESS_TOKEN_EXPIRE_MINUTES` | 可选配置，有默认值 |

开发环境可复制 `backend/.env.example`；生产环境需通过 Vault/Kubernetes Secret/Docker env_file 注入，并确保 `data/uploads`、`data/posts` 均挂载到持久化卷。
